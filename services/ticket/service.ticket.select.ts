...
export async function getTicketById(id: number, connectionToUse?: PoolConnection): Promise<ITicket | undefined>
{
    const query = `SELECT * FROM ${TABLE} WHERE id = ?;`;

    return await executeSelectQuery<ITicket>(query, [id], connectionToUse);
}

export async function getAllTickets(from: number = 0,
                                    limit: number = MAX_NUM_OF_TICKETS, connectionToUse?: PoolConnection) : Promise<ITicket[] | undefined>
{
    const query = `SELECT * FROM ${TABLE} LIMIT ?, ?;`;

    return await executeSelectQuery<ITicket[]>(query, [from, limit], connectionToUse);
}
                                                     ...
                                                     limit: number = MAX_NUM_OF_TICKETS, connectionToUse?: PoolConnection) : Promise<ITicket[] | undefined>
{
    ...
    return await executeSelectQuery<ITicket[]>(query, [hasBeenPaid, from, limit], connectionToUse);
}
                                                     ...
                                                     limit: number = MAX_NUM_OF_TICKETS, connectionToUse?: PoolConnection) : Promise<ITicket[] | undefined>
{
    ...
    return await executeSelectQuery<ITicket[]>(query, [paymentMethod, from, limit], connectionToUse);
}
                                                    ...
                                                    limit: number = MAX_NUM_OF_TICKETS, connectionToUse?: PoolConnection): Promise<ITicket[] | undefined>
{
    ...
    return await executeSelectQuery<ITicket[]>(query, [sponsorLevel, from, limit], connectionToUse);
}
                                                 ...
                                                 limit: number = MAX_NUM_OF_TICKETS, connectionToUse?: PoolConnection): Promise<ITicket[] | undefined>
{
    ...
    return await executeSelectQuery<ITicket[]>(query, [shirtSize, from, limit], connectionToUse);
}
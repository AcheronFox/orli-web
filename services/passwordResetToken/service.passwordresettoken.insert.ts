...
export async function createPasswordToken(token: IPasswordResetToken): Promise<number>
{
    const query = `INSERT INTO ${TABLE} SET ?;`;

    return executeInsertQuery(query, [token]);
}
...
export async function replacePasswordToken(arg1: IPasswordResetToken | number, newToken: IPasswordResetToken): Promise<number>
{
    ...
    return await createPasswordToken(newToken);
}
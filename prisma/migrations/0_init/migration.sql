...
-- CreateTable
CREATE TABLE `dailyTicket` (
    `id` INTEGER NOT NULL,
    `currentValidity` DATE NOT NULL,
    `pastValidities` VARCHAR(512) NULL,
...
CREATE TABLE `room` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `building` ENUM('Facan', 'Vidra', 'Kocsag') NOT NULL,
...
-- AddForeignKey
ALTER TABLE `attendee` ADD CONSTRAINT `fk_attendee_dailyTicket1` FOREIGN KEY (`dailyTicketId`) REFERENCES `dailyTicket`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
